...
test('child logger inherits and extends standing context', () => {
  const logger = makeLogger('child');
  const providerScope = logger.child('sources', { provider: 'Hindmoviez', extension: 'Phisher' });
  providerScope.info('started');
  providerScope.child({ sourceId: 'abc' }).info('resolved');
  logger.flush();
