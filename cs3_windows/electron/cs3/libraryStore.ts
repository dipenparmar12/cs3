...
export class LibraryStore {
  ...
  /**
   * Records the exact stream that delivered playback, with the query to
   * re-resolve it when its link expires.
   *
   * A source is recorded as working only after playback has genuinely run for a
   * few seconds — not when it was selected, and not when a URL was merely
   * attached. A release that opens and dies on the second GOP looks identical
   * to a good one at attach time, and saving it would send the viewer straight
