...
| 41 | [41-desktop-extension-platform-and-community-standard.md](41-desktop-extension-platform-and-community-standard.md) | **The Desktop Extension Platform, and a community standard of our own** — the five-lane provider model (`.cs3` / upstream cross-platform `.csj` jar / our `.csx` / Stremio addon URL / yt-dlp rule), CSX-REPO + CSX-EXT + CSX-SDK, ed25519 signing with pinned keys, the capability sandbox, the unified metadata model (multi-source ratings, crew, chapters, typed artwork), the developer CLI and fixture harness, and the publishing/governance model. Expands doc 39; supersedes doc 27 §6–§9 |
| 42 | [42-resume-continuity-and-seekable-sources.md](42-resume-continuity-and-seekable-sources.md) | **Resume, continuity, and sources that cannot seek** — why continuing where you left off is two features rather than one: a resume path that bypasses `seekTo` and fails silently on three of five transports, and a large class of provider hosts (`video-downloads.googleusercontent.com` and friends) that answer every `Range` with byte zero. Declares seekability as a capability, adds the honest UI for when it is absent, a bounded stream cache that makes a forward-only origin seekable, and Android-parity auto-continue. **Proposed; nothing built** |
