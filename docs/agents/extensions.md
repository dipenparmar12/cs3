...
- **Windows will not replace an archive a class loader holds (2026-09-24).** `PluginHost.load` dropped the loader of a load that failed — never tracked, so `unload` could not close it — and it held the `.cs3` open for the life of the JVM: `EPERM … rename 'Ultima…cs3.tmp' -> 'Ultima…cs3'`, twice in one session, for the extension that fails at `load()` every launch. The sidecar now closes it (and replaces, not leaks, a second load of the same id; `beforeUnload` lookups catch `LinkageError`; `jar:` URL caching is off so a plugin's own `getResource().openStream()` cannot pin the file). **`unload` withdraws the plugin's `apis`/`allProviders`/`extractorApis` entries by `sourcePlugin`**, exactly as upstream's `unloadPlugin` — before this an updated extension's old extractors stayed first in `extractorApis` behind a closed loader. Belt and braces on the host: `archivePlacement.ts` retries ~1.5s, then installs **beside** the held file and points the record at it; the held copy is swept from `extension_displaced_archives` once released. An in-flight activation of the same extension is awaited before unloading, or it finishes afterwards, marks the old version live and the reload skips the new bytes.
- **Automatic by default, as on Android** — see AGENTS.md "Updates install automatically". A republish reads "v10 updated to its maintainer's latest build", never "from v10 to v10".
...
**First launch bootstraps repositories** (`cs3/bootstrap.ts`, background, progress shown, once per `BOOTSTRAP_VERSION`, capped at `PLUGINS_PER_REPOSITORY` = 16, never blocks catalogues/indexers). `bundled: true` means `provider-e2e.mjs` has driven that repo end-to-end. **What it installs is chosen by language** (`cs3/starterPlugins.ts`): the system locale's language and English, then `multi`/undeclared, working before beta before slow, nothing marked down — it used to take the first twelve of each index in list order, which in the mixed repositories meant Hindi, Tamil and Turkish scrapers first. A bundled repository in another language (German Providers on an English install) is not fetched at all. `recloudstream/extensions` passed the harness (5 loaded, 4 answering, bytes from YouTube) and is **deliberately not bundled**: Dailymotion, Invidious, Twitch and YouTube would put channels and trailers into every film search.
